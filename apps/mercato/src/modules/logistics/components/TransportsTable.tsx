...
function formatDateRange(start: string | null, end: string | null): string {
  ...
  const startTime = new Intl.DateTimeFormat(undefined, { timeStyle: 'short' }).format(date)
  if (!end) return `${day}, ${startTime}`
  const endTime = new Intl.DateTimeFormat(undefined, { timeStyle: 'short' }).format(new Date(end))
  return `${day}, ${startTime}–${endTime}`
}

function formatMoney(value: number | null): string {
  if (value == null) return '—'
  return new Intl.NumberFormat(undefined, { style: 'currency', currency: 'PLN', maximumFractionDigits: 0 }).format(value)
}
...
export function TransportsTable() {
  ...
  const columns = React.useMemo<ColumnDef<TransportRow>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => formatDateRange(row.original.pickupWindowStart, row.original.pickupWindowEnd),
      meta: { priority: 2 },
    },
    {
      id: 'cargo',
    ...
    },
    {
      ...
      enableSorting: false,
      cell: ({ row }) => formatMoney(row.original.clientPrice),
      meta: { priority: 4 },
    },
    {
      id: 'carrier',
      ...
      cell: ({ row }) => row.original.carrier ? (
        <div className="flex flex-col items-start gap-1">
          ...
          <Badge variant={badgeVariant(row.original.carrier.status)} size="sm" dot>
  ...
  }, [filters.carrierStatus, page, search, sorting, t])
  ...
  if (error && rows.length === 0 && !loading) {
    return (
      <ErrorMessage
        label={error}
        action={<Button size="sm" variant="outline" onClick={() => setReloadToken((value) => value + 1)}>{t('logistics.actions.retry')}</Button>}
      />
    )
  }
  ...
  return (
    <DataTable<TransportRow>
      extensionTableId="logistics.transports"
      title={(
        <span className="flex flex-col">
          <span>{t('logistics.transports.title')}</span>
          <span className="text-sm font-normal text-muted-foreground">{t('logistics.transports.description')}</span>
        </span>
      )}
      titleHeadingLevel={1}
      columns={columns}
